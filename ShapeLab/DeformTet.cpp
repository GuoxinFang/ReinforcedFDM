...
bool DeformTet::RunIteration(int loop)
{
...
void DeformTet::Initialization()
{
	...
	for (GLKPOSITION Pos = TetMesh->GetTetraList().GetHeadPosition(); Pos;) {
		...
		//below are the test for new SORO proposal
		bool selected = false;
		for (int i = 0; i < 4; i++) {
			if (Tet->GetFaceRecordPtr(i + 1)->selected) selected = true;
		}
		if (selected) {
			Tet->selected = true; Tet->weight = proposalYoungs;
		}
		else {
			Tet->selected = false; Tet->weight = 1;
		}
	}
...
void DeformTet::ComputeLocalGoalAndInverse()
{
	...
	for (GLKPOSITION Pos = TetMesh->GetTetraList().GetHeadPosition(); Pos;) {
		...
		for (int i = 0; i < VELE; i++) {
			if (Tet->GetFaceRecordPtr(i + 1)->selected){
				Tet->selected = true;
				break;
			}
		}
		...
		/*if (Tet->selected) {
			for (int i = 0; i < VELE; i++) {
				for (int j = 0; j < 3; j++) { P(j, i) *= pow(1.2, 1.0 / 3); }
			}
		}*/
		if (!Tet->selected) {
			for (int i = 0; i < VELE; i++) {
				for (int j = 0; j < 3; j++) { P(j, i) *= pow(proposalPossion, 1.0 / 3); }
			}
		}
...
{ 
	...
	for (GLKPOSITION Pos = TetMesh->GetTetraList().GetHeadPosition(); Pos;) {
		...
		if (Tetra->GetIndexNo() < 0) continue; //Here need better consideration
		...
		for (int i = 0; i < VELE; i++) {
			...
			for (int j = 0; j < VELE; j++) {
				//if (vdxArr[j] < 0) continue; //Here need better consideration.
				/*if (i == j) matA.insert(fdx + j, vdxArr[i]) = c2*pow(1, FACTOR);
				else matA.insert(fdx + j, vdxArr[i]) = c1*pow(1, FACTOR);*/
				if (i == j) matA.insert(fdx + j, vdxArr[i]) = c2 * Tetra->weight;
				else matA.insert(fdx + j, vdxArr[i]) = c1 * Tetra->weight;
			}
		}
	}
	matA.makeCompressed();
		...
		for (int omptime = 0; omptime < Core; omptime++) {
			...
			for (GLKPOSITION Pos = TetMesh->GetTetraList().GetHeadPosition(); Pos;) {
				...
				if (Tetra->GetIndexNo() != -1) {
					...
					for (int i = 0; i < VELE; i++) {
						for (int j = 0; j < 3; j++) {
							//VectorBSide[j](fdx*VELE + i) = (LocalCoord[fdx](j, i) - center[j])*pow(1, FACTOR);
							VectorBSide[j](fdx*VELE + i) = (LocalCoord[fdx](j, i) - center[j])*Tetra->weight;
						...
						for (int k = 0; k < VELE; k++) {
							...
							if (node->GetIndexNo() < 0) {
								...
								/*if (i == k) for (int l = 0; l < 3; l++) VectorBSide[l](fdx *VELE + i) -= p[l] * c2*pow(1, FACTOR);
								else for (int l = 0; l < 3; l++) VectorBSide[l](fdx *VELE + i) -= p[l] * c1*pow(1, FACTOR);*/

								if (i == k) for (int l = 0; l < 3; l++) VectorBSide[l](fdx *VELE + i) -= p[l] * c2*Tetra->weight;
								else for (int l = 0; l < 3; l++) VectorBSide[l](fdx *VELE + i) -= p[l] * c1*Tetra->weight;
							}
						}
					}
				}
...
bool DeformTet::UpdateVertex()
{
	for (GLKPOSITION Pos = TetMesh->GetNodeList().GetHeadPosition(); Pos;) {
		...
		if (idx > -1)
			...
			node->SetCoord3D(VectorXPosition[0](idx), VectorXPosition[1](idx), pp[2]);
	}
	return true;
}
...
void DeformTet::moveHandleRegion() {
	...
	//		node->GetCoord3D(pp[0], pp[1], pp[2]);
	//		//pp[1] -= 20; pp[2] -= 3;
	//		double yy = pp[1], zz = pp[2];
	//		pp[1] = yy*cos(angle) + sin(angle)*zz;
	//		pp[2] = -yy*sin(angle) + cos(angle)*zz;
	//		//pp[1] += 20; pp[2] += 3;
	//		node->SetCoord3D(pp[0], pp[1], pp[2]);
	//	}
	//}
