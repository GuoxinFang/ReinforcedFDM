...
void QMeshPatch::deleteFace(QMeshFace *face) {
	if (faceList.RemoveAt(faceList.Find(face)) != face) printf("Cannot delete face?\n");
	for (int j = 1; j <= face->GetEdgeNum(); j++) {
		QMeshEdge * edge = face->GetEdgeRecordPtr(j);
		if (!edge) continue;
		//will be an unreference edge
		if (edge->GetFaceList().GetCount() <= 1) {
			edgeList.RemoveAt(edgeList.Find(edge));
			for (int k = 0; k<2; k++) {
				QMeshNode *node = edge->GetStartPoint();
				if (k == 1) node = edge->GetEndPoint();
				//will be an unreference node
				if (node->GetEdgeNumber() <= 1) {
					nodeList.RemoveAt(nodeList.Find(node));
					delete node;
				}
				else { //undate the node
					node->GetEdgeList().Remove(edge);
					node->GetFaceList().Remove(face);
					//node->inner=false;
				}
			}
			delete edge;
			...
			edge->GetFaceList().Remove(face); //edge->inner=false;
			edge->GetStartPoint()->GetFaceList().Remove(face);
			edge->GetEndPoint()->GetFaceList().Remove(face);
